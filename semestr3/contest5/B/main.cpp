...
Board GetMaxBoard(const std::vector<long long int>& partial_sum_array, const std::vector<Board>& tree_max_sum_in_row, long long int left, long long int right, long long int next_pow2) {
    ...
    long long int sum_between = partial_sum_array[tree_max_sum_in_row[right].left] - partial_sum_array[tree_max_sum_in_row[left].right+1];
...
//    std::cout << "right_bpard : " << right_board.left << ' ' << right_board.right << ' ' << right_board.sum << '\n';
    ...
    if (all_sum > left_board.sum && all_sum > right_board.sum) { // left < right-1 &&
...
//    Prlong long int(partial_sum_array);
//    std::cout << "row\n";
//    Prlong long int(row);
...
//    Prlong long int(tree_max_sum_in_row);

    long long int ind = next_pow2 - 1;
    do {
        long long int start_ind = ind = Parent(ind);
        for (long long int i = ind; i < start_ind * 2 + 1; ++i) {
